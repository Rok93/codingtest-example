...
public class KakaoProblem3_2 {
    ...
    private List<Frame> list;//프레임 정보를 저장할 리스트
    ...
    //요구 사항을 충족하는지 확인
    static boolean check(List<Frame> list) {
            ...
            switch (frame.type) {//프렘의 타입
                ...
                case BOARD://보일 때
                    ...
                    if (list.contains(new Frame(frame.x, frame.y - 1, 0))
                            ...
                            || (list.contains(new Frame(frame.x - 1, frame.y, 1)) && list.contains(new Frame(frame.x + 1, frame.y, 1))))
                        continue;
                    else return false;
